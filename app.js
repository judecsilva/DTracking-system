...
db.version(4).stores({
    settings: '++id, targetAmount, adminPassword, synced',
    staff: '++id, name, routeName, phone, password, synced',
    dailyIssues: '++id, staffId, date, [date+staffId], synced',
    dailySales: '++id, staffId, date, [date+staffId], synced'
});
...
const SyncManager = {
    ...
    async init() {
        setInterval(() => this.performSync(), 15000); // Sync every 15s
        this.performSync();
    ...
    updateStatus(state) {
        ...
        if(state === 'syncing') {
            dot.className = 'sync-dot w-2 h-2 rounded-full bg-blue-500 animate-pulse';
            text.innerText = 'Syncing...';
        } else if(state === 'online') {
            dot.className = 'sync-dot w-2 h-2 rounded-full bg-emerald-500';
            text.innerText = 'Cloud Synced';
        } else if(state === 'offline') {
            dot.className = 'sync-dot w-2 h-2 rounded-full bg-rose-500';
            text.innerText = 'Offline Mode';
        } else if(state === 'error') {
            dot.className = 'sync-dot w-2 h-2 rounded-full bg-orange-500';
    ...
    async syncSettings() {
        ...
        for (const s of local) {
            ...
            if(!error) await db.settings.update(s.id, { synced: 1 });
        }
    },
    ...
    async syncStaff() {
        ...
        for (const s of local) {
            ...
            if(!error && data && data[0]) {
                await db.staff.update(s.id, { synced: 1, cloud_id: data[0].id });
            }
        }
    ...
    async syncIssues() {
        const local = await db.dailyIssues.where('synced').notEqual(1).toArray();
        for (const r of local) {
            ...
            const { error } = await supabaseClient
                ...
                .upsert({
                    ...
                    card95: r.card95,
                    card96: r.card96,
                    reload_val: r.reloadVal
                });
                
            if(!error) await db.dailyIssues.update(r.id, { synced: 1 });
        }
    },

    async syncSales() {
        const local = await db.dailySales.where('synced').notEqual(1).toArray();
        for (const r of local) {
            const staff = await db.staff.get(r.staffId);
            if(!staff || !staff.cloud_id) continue;
            ...
            const { error } = await supabaseClient
                ...
                .upsert({
                    staff_id: staff.cloud_id,
                    date: r.date,
                    sold_card48: r.soldCard48,
                    sold_card95: r.soldCard95,
                    sold_card96: r.soldCard96,
                    sold_reload_cash: r.soldReloadCash,
                    returned_card48: r.returnedCard48,
                    returned_card95: r.returnedCard95,
                    returned_card96: r.returnedCard96,
                    hand_cash: r.handCash,
                    shortage_amt: r.shortageAmt,
                    total_commission: r.totalCommission
                });
                
            if(!error) await db.dailySales.update(r.id, { synced: 1 });
        }
    }
};
...
function setupEventListeners() {
    ...
    document.getElementById('staff-form').addEventListener('submit', async (e) => {
        ...
        if(id) {
            ...
            await db.staff.update(Number(id), {name, routeName, phone, password, target, synced: 0});
            ...
            await db.staff.add({name, routeName, phone, password, target, synced: 0});
    ...
    document.getElementById('target-form').addEventListener('submit', async (e) => {
        ...
        if(first) {
            ...
            await db.settings.add({targetAmount, workingDays, adminPassword, synced: 0});
        }
